...
switch($op){
    ...
    //呈現修改地圖表單
    case "modify_form":
    $content=bootstrap(add_map_form($map_sn),map_js_link($map_sn),$css_link,$js_fun);
    break;
...
function add_map_form($map_sn=""){
    ...
    if(empty($map_sn)){
        ...
        <input type='hidden' name='map_sn' value='{$map_sn}'>  
        ";
    }
    $main="
...
function update_map(){
    global $link,$tblBook;
    $sql="replace into `google_map_book` (`map_sn`, `map_title`, `map_content`, `map_lat`, `map_lng`, `map_zoom`) values ('{$_POST['map_sn']}','{$_POST['map_title']}','{$_POST['map_content']}','{$_POST['map_lat']}','{$_POST['map_lng']}','{$_POST['map_zoom']}')";
    //執行資料庫查詢
